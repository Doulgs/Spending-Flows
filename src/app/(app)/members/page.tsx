  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {canManage && <>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-0">
            {loading ? <div className="space-y-px">{Array.from({ length: 3 }, (_, index) => <Skeleton key={index} className="h-20 rounded-none" />)}</div> : members.map((member) => (
              <div key={member.id} className="group flex items-center gap-4 border-b border-border px-5 py-4 last:border-0 hover:bg-muted/30">
                ...
                <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{member.display_name}</p><p className="truncate text-xs text-muted-foreground">{member.email}</p></div>
        ...
        {invitations.length > 0 && <Card className="overflow-hidden"><CardHeader className="border-b border-border"><CardTitle>Convites pendentes</CardTitle><CardDescription>Pessoas que ainda não aceitaram o acesso.</CardDescription></CardHeader><CardContent className="p-0">{invitations.map((invitation) => <div key={invitation.id} className="flex items-center gap-4 border-b border-border px-5 py-4 last:border-0"><div className="flex size-10 items-center justify-center rounded-lg border border-border bg-muted/40"><MailPlus className="size-4 text-muted-foreground"/></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{invitation.email}</p><p className="text-xs text-muted-foreground">Enviado {formatDistanceToNow(new Date(invitation.created_at), { addSuffix: true, locale: ptBR })}</p></div><RoleBadge role={invitation.role}/><Button variant="ghost" size="sm" className="text-muted-foreground" onClick={() => revokeInvitation(invitation)}>Revogar</Button></div>)}</CardContent></Card>}
      </>}
    </div>
  );
}
