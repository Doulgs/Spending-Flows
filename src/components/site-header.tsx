...
export function SiteHeader({ title }: { title: string }) {
  ...
  return <>
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-2 border-b border-border/80 bg-background/80 px-4 backdrop-blur-xl lg:px-6">
      ...
      <div className="ml-auto flex items-center gap-1.5">{canManage && <DropdownMenu><DropdownMenuTrigger asChild><Button size="sm" className="h-8"><Plus/>Novo</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => openQuickAdd("expense")}>Nova despesa</DropdownMenuItem><DropdownMenuItem onClick={() => openQuickAdd("income")}>Nova receita</DropdownMenuItem><DropdownMenuItem onClick={() => openQuickAdd("transfer")}>Nova transferência</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}<Button size="icon" variant="ghost" className="size-8"><Bell/><span className="sr-only">Notificações</span></Button></div>
    </header>
